...
Both of these are more advanced, one-time setup steps (internet tunnel
options under the **INTERNET** tab; a virtual-COM-port bridge for
running CW Skimmer / Ham Radio Deluxe / Logger32 alongside HAMCTRL,
requiring [com0com](https://com0com.sourceforge.net/) on the client
machine). See the in-app help text on those tabs — covering every option
here would make this guide longer than it needs to be for a first
install.
...
To już bardziej zaawansowane, jednorazowe ustawienia (opcje tunelu
internetowego w zakładce **INTERNET**; mostek wirtualnych portów COM do
uruchomienia CW Skimmer / Ham Radio Deluxe / Logger32 równolegle z
HAMCTRL, wymaga [com0com](https://com0com.sourceforge.net/) na
komputerze klienckim). Zobacz tekst pomocy w samej aplikacji przy tych
zakładkach — opisanie tu każdej opcji wydłużyłoby ten poradnik ponad
potrzeby pierwszej instalacji.
