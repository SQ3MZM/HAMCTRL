...
class App:
    ...
    async def api(self, method: str, path: str, body: dict,
        ...
        if p == "/api/rig/features" and method == "GET":
            ...
            # typu BK-IN"), REVISED same day: the first version of this fix
            # made ONLY role=="admin" ever get the admin-shaped response
            # (every detected dynamic action/slider, including DISABLED
            # ones, each tagged with its enabled state - meant for the
            # config table). That broke a real, intended feature: the user
            # confirmed "settings" is a DELIBERATE admin-delegated grant -
            # an operator with it is meant to fully use KONFIGURACJA
            # (including FUNKCJE RADIA), same as an admin would, not a
            # narrower "can see their own audio device" permission.
            #
            # The actual bug was narrower: radiofunctions.js's regular
            # RADIO-tab panel (the LIVE control surface) and admin.js's
            # KONFIGURACJA->FUNKCJE RADIA config table both call this same
            # endpoint, and the response shape alone tells the frontend
            # which one it's rendering into - so whichever shape a
            # settings-holder received leaked into BOTH places at once.
            # Fixed by making the caller's INTENT explicit instead:
            # admin.js's config-page fetch now passes ?view=admin (see
            # loadRigFeatures() in admin.js) - only THAT gets the full
            # shape for a settings-holding non-admin. radiofunctions.js's
            # plain fetch (no param) never does, so the live RADIO-tab
            # panel is now always the filtered view unless the caller is
            # a real admin (who reasonably sees/can-toggle everything
            # everywhere anyway, same as before this whole investigation).
            view_param = query.get("view", "")
            if isinstance(view_param, list):
                view_param = view_param[0] if view_param else ""
            wants_admin_view = (view_param == "admin")
            _admin_view = "admin" if (role == "admin" or
                                       (wants_admin_view and self._has_perm(uid, role, "settings"))) else role
